...
%w(manage_news_entries manage_pages manage_recruitments manage_messages manage_permissions manage_users manage_settings manage_forums_settings).each do |permission|
  Permission.where(identifier: permission).first_or_create(name: permission.humanize)
end

%w(officer raider trial casual).each do |user_group|
...
%w(application_template application_board roster_api_url roster_raider_ranks roster_armory_link roster_cache_expire progress_reference_url progress_raids progress_cache_expire npc_image_url).each do |identifier|
